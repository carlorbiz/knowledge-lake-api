/**
 * CORRECTED FIX: Enhanced generateCourseRecommendation() Function
 * 
 * FIXES FUNCTION NAMING ISSUES:
 * - Uses callGemini() not callGemini_()
 * - Maintains compatibility with your existing function names
 * 
 * DEPLOYMENT: Replace your entire generateCourseRecommendation() function with this corrected version
 */
...
• PDF content takes priority over general knowledge - use provided materials first`;
  ...
  // CORRECTED: Call Gemini with proper function names
  let rec;
  if (srcPack.pdfFiles && srcPack.pdfFiles.length > 0) {
    ...
    rec = callGemini(mappingPrompt, 8000); // CORRECTED: callGemini() not callGemini_()
  }
 ...
 * 
 * KEY CORRECTIONS:
 * - Uses callGemini() (your correct function name)
 * - Includes fallback for callGeminiWithPDFs if not available
 * - Maintains compatibility with your existing function structure
 * - Enhanced prompting for PDF integration
