 ...
 */
function generateAudioFromScript(voiceScript, voiceType = 'professional', slideName = 'slide') {
  try {
...
${scriptText}`;
  ...
  if (data.candidates && data.candidates[0] && data.candidates[0].content && data.candidates[0].content.parts[0].inlineData) {
    ...
    return Utilities.newBlob(audioBytes, 'audio/wav');
  } else {
...
function saveAudioToDrive(audioBlob, fileName) {
  try {
    ...
    console.error('DRIVE_FOLDER_ID value:', DRIVE_FOLDER_ID);
...
function doPost(e) {
  try {
    ...
    const voiceType = requestData.voiceType || 'professional';
...
function testAudioGeneration() {
  ...
  const result = generateAudioFromScript(testScript, 'professional', 'test_slide');
