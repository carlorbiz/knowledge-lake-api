...
function onOpen() {
  ...
  ui.createMenu('🎙️ Audio Generation')
    ...
    .addSubMenu(ui.createMenu('🔄 Auto-Process by Status')
      ...
      .addItem('🔄 Process All "Pending" Slides (Batch)', 'processPendingSlides')
 ...
 * - Rich Voiceover Script (300-450 words)
 * - Image Prompt
 */
...
**2. VOICEOVER SCRIPT (300-450 words):**
...
- 300-450 words for optimal timing (2-3 minutes audio)
...
Respond ONLY with valid JSON. No other text.`;
    ...
    if (result.candidates && result.candidates[0].content.parts[0].text) {
      ...
      Logger.log('Content enhancement failed: ' + JSON.stringify(result));
      return null;
    }

  } catch (error) {
    Logger.log('Error enhancing content: ' + error.message);
    return null;
  }
}
...
function generateAudio(script, voiceName, targetAudience, slideTitle) {
  try {
    // Get voice prompt
    const voicePrompt = getVoicePrompt(voiceName, targetAudience);

    // Build TTS request for Gemini
    const requestBody = {
      "contents": [{
        "parts": [{
          "text": `${voicePrompt}\n\nSlide Title: ${slideTitle}\n\nVoiceover Script:\n${script}`
    ...
    const response = UrlFetchApp.fetch(CONFIG.TTS_ENDPOINT, options);
    const result = JSON.parse(response.getContentText());
